 ...
 *
 * Opciones:
 *   --depth N            profundidad máxima (0 = Auto: solo manda el tiempo). Por defecto 0 (Auto)
 *   --time S             tiempo máximo por jugada en segundos. Por defecto 5
 *   --no-enclosure       activa la regla "No enclosure" (por defecto: noEnclosure=false, se permite encerrar;
*                        con la regla desactivada puede haber tablas por encierro mutuo — ver "mutual-enclosure")
 ...
 *   --current DIR        carpeta del motor actual (WebAssembly). Por defecto: la carpeta que contiene a esta
 *                        (..), es decir ~/opt/selfo si el script vive en ~/opt/selfo/analysis
 *   --legacy DIR         carpeta del motor anterior. Por defecto: ../legacy (relativo a esta carpeta)
 *   --out DIR            carpeta donde guardar las partidas. Por defecto:
 *                        ./games/selfplay/<combinación de parámetros>/ (ver más abajo). Si se indica --out,
 ...
 * Pensado para vivir en ~/opt/selfo/analysis/ (junto a enumerate_boards.js),
 * con los motores en ~/opt/selfo/ y ~/opt/selfo/legacy/. Cada partida
 * individual (no cada pareja) se guarda en su propio fichero, por defecto en:
 *   analysis/games/selfplay/r<radius>_p<pieces>_d<depth>_t<time>[_opción...]/
 * Los cuatro parámetros principales (radius, pieces, depth, time) van SIEMPRE
 * en el nombre. --no-enclosure, --max-moves, --first-move-depth, --strategy
 * y --seed se añaden solo si se especifican explícitamente en la línea de
 * comandos. Si la carpeta ya tiene partidas de una ejecución anterior con los
 * mismos parámetros, las nuevas se numeran a continuación (no se sobrescriben).
 */
const fs = require("fs");
...
function parseArgs(argv) {
  const opts = { depth: 0, time: 5, noEnclosure: false, maxMoves: 200, firstMoveDepth: 1, strategy: undefined,
    seed: null, current: path.join(__dirname, ".."), legacy: path.join(__dirname, "..", "legacy"), out: null, verbose: false };
  const given = new Set();
  const pos = [];
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (a) {
      ...
      case "--seed": opts.seed = Number(next()); given.add(a); break;
 ...
 *  explícitos, dentro de analysis/games/selfplay/ (o donde apunte --out). */
function resolveOutDir(opts) {
  ...
  return path.join(__dirname, "games", "selfplay", name);
}
function usage(code) {
...
function playGame(board, engines, referee, opts) {
  ...
  const enc = !opts.noEnclosure; // enclosureAllowed: true = se permite encerrar (valor por defecto de este script)
  let turn = "black", plies = 0;
  const moves = [];
  const finish = (winner, reason) => ({ winner, reason, plies, moves });
  ...
  while (true) {
    ...
    moves.push({ ply: plies, color: turn, engine: eng.name, from: mv.from, to: mv.to, depthReached: res.depthReached, nodesEvaluated: res.nodesEvaluated, score: res.score, ms: Math.round(ms * 10) / 10 });
...
function main() {
  ...
  const existing = fs.readdirSync(outDir).map((f) => /^partida_(\d+)\.json$/.exec(f)).filter(Boolean).map((m) => Number(m[1]));
  const startIndex = existing.length ? Math.max(...existing) + 1 : 1;
  const pad = String(startIndex + opts.games).length;

  console.log(`Partidas=${opts.games} radius=${opts.radius} pieces=${opts.pieces} depth=${opts.depth || "Auto"} time=${opts.time}s ` +
    `enclosure=${opts.noEnclosure ? "restringido (regla activa)" : "permitido (por defecto)"} max-moves=${opts.maxMoves} seed=${opts.seed}`);
  console.log(`  wasm: ${path.resolve(opts.current)}\n  js  : ${path.resolve(opts.legacy)}`);
  console.log(`  carpeta de esta ejecución: ${path.resolve(outDir)}${existing.length ? ` (ya tenía ${existing.length} partidas; se numera desde ${startIndex})` : ""}\n`);

  let board = null, blackKeysInit = null, whiteKeysInit = null, tAll = performance.now();
  for (let g = 0; g < opts.games; g++) {
    const second = g % 2 === 1;                    // 2ª partida de la pareja: colores intercambiados
    if (!second) {
      board = build(opts.radius, opts.pieces, !opts.noEnclosure);
      blackKeysInit = []; whiteKeysInit = [];
      for (const [k, c] of board.cells) (c.color === "black" ? blackKeysInit : c.color === "white" ? whiteKeysInit : null)?.push(k);
    }
    ...
    const idx = startIndex + g;
    const record = {
      index: idx, radius: opts.radius, pieces: opts.pieces, seed: opts.seed,
      ...
      pair: Math.floor(g / 2) + 1, engines: { black: engines.black.name, white: engines.white.name },
      initial: { black: blackKeysInit, white: whiteKeysInit },
      result: { winner: r.winner, reason: r.reason, plies: r.plies },
      moves: r.moves,
    };
    fs.writeFileSync(path.join(outDir, `partida_${String(idx).padStart(pad, "0")}.json`), JSON.stringify(record, null, 1));
  ...
  console.log(`\nPartidas guardadas en: ${path.resolve(outDir)}`);
  console.log(`Tiempo total: ${secs.toFixed(1)} s`);
